...
The backend REST API was implemented using Node.js, Express, and TypeScript.  
The system uses a MySQL relational database with properly defined tables, primary keys, and foreign key constraints.

The API supports full CRUD (Create, Read, Update, Delete) operations for both Resources and Events.  
All endpoints were tested using Postman, and all database changes were verified using MySQL Workbench.

The implemented solution aligns with the REST API design defined in Milestone 2.

---

## Design Updates Since Milestone 2

During implementation, the following refinements were made:

- Finalized the MySQL schema with appropriate data types.
- Implemented primary keys with auto-increment.
- Enforced foreign key relationship between `events.resource_id` and `resources.resource_id`.
- Added `image_url` field to both tables to store external image links instead of image files.
- Completed full CRUD functionality for both Resources and Events.
- Implemented search and filtering endpoints.
- Applied strict TypeScript typing for request bodies, parameters, and responses.

The delivered backend matches the documented REST API design. No structural deviations exist.
...
| ---------------------- | --------------- | ----------------------------- |
| Resources CRUD         | Completed       | Fully tested                  |
| Events CRUD            | Completed       | Fully tested                  |
| Search Endpoints       | Completed       | Name and title search         |
| Filter Endpoints       | Completed       | Category, date, availability  |
| Foreign Key Constraint | Implemented     | Ensures referential integrity |
| Image URL Support      | Implemented     | External URLs stored          |
| Authentication         | Not Implemented | Not required                  |
| Pagination             | Pending         | Future enhancement            |
...
- GET `/api/resources`
- GET `/api/resources?resourceId={id}`
- POST `/api/resources`
- PUT `/api/resources`
- DELETE `/api/resources/{id}`
- GET `/api/resources/search/name/{keyword}`
- GET `/api/resources/category/{category}`
- GET `/api/resources/availability/{status}`
...
- GET `/api/events/resource/{resourceId}`
...
- Database modification results

Each Create, Update, and Delete request was followed by a GET request to confirm database changes.

---

## Database Validation

All database operations were verified using MySQL Workbench.

- INSERT operations confirmed using SELECT queries.
- UPDATE operations verified by retrieving modified records.
- DELETE operations confirmed by checking record removal.
- Foreign key constraints ensure valid relationships between tables.

This demonstrates proper relational database management and data integrity.
